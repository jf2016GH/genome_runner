...
import random
import traceback
import bin.bedfilecreator as uscsreader
import argparse

lookup = TemplateLookup(directories=["frontend/templates"])
...
class WebUI(object):
	...
	def index(self):
		if DEBUG_MODE or not organism in self._index_html:
			...
			paths.traverse(os.path.join(sett["dir_data"],sett["default_organism"]))
...
if __name__ == "__main__":
	global static_dir,results,media
	static_dir = os.path.abspath(os.path.join(".", "frontend/static"))
	...
	parser.add_argument("--port", nargs="?", help="Socket port to start server on. Required to start server.") 
	...
	if sett["dir_data"] == "":
		print "ERROR: Database directory not set. Run bedfilecreator in directory that the database should be located in."
		sys.exit()
	if not os.path.exists(sett["dir_data"]):
		print "ERROR: {} does not exist.".format(sett["default_organism"],sett["dir_data"],os.path.join(sett["dir_data"],sett["default_organism"]))
		sys.exit()
	if not os.path.exists(sett["dir_data"],sett["default_organism"]):
		print "ERROR: Database for default organism {} does not exist. Either change the default organism or add data for that organism to the database at {} using the bedfilecreator".format(sett["default_organism"],sett["dir_data"])
		sys.exit()
