...
def output_p_value(foi_obs, n_fois, bg_obs, n_bgs, foi_path, gf_path, background_path, detailed_outpath,
				   run_randomization_test=False, stat_test=None, progress=None):
	...
	sign, pval, odds_ratio, shrunken_or, ci_lower, ci_upper = calculate_p_value_odds_ratio(foi_obs, n_fois, bg_obs,
																						   n_bgs, foi_name, gf_path,
																						   stat_test=stat_test,
																						   background_path=background_path,
																						   run_files_dir=run_files_dir,
																						   progress=progress)
...
def p_rand(foi_path, n_fois, background_path, bg_obs, n_bgs, gf_path, progress=None, run_files_dir=None):
	...
	p_rand = [1]
	for r in rnd_stats:
		sign, pval, odds_ratio, _, _, _ = calculate_p_value_odds_ratio(r["intersectregions"], r["queryregions"], bg_obs,
																	   n_bgs, base_name(foi_path), gf_path,
																	   stat_test='chisquare', progress=None,
																	   run_files_dir=None)
		p_rand.append(pval)
			...
			if len([x for x in data_dirs if x in f]) == 0:
				...
				if validate_rsids(out_f):
					...
					if not os.path.exists(rsid_path):
						logger.error(
							'rsID conversion not available for this organism. Feature set {} removed'.format(f))
						continue
		...
		for out_f in file_paths:
			...
			if len([x for x in data_dirs if x in out_f]) == 0:
				...
				if validate_rsids(unzipped_f):
					...
					rsid_path = os.path.join(root_data_dir, 'custom_data', 'rsid_conversion', organism)
					if not os.path.exists(rsid_path):
						logger.error('rsID conversion not available for this organism. Feature set {} removed'.format(
							unzipped_f))
						continue
					files = [x for x in os.listdir(rsid_path) if
							 os.path.isfile(os.path.join(rsid_path, x)) and x.endswith('.bed')]
...
def run_hypergeom(fois, gfs, bg_path, outdir, job_name="", zip_run_files=False, bkg_overlaps_path="", root_data_dir="",
				  run_annotation=True, run_randomization_test=False, pct_score="", organism="", stat_test=None):
	global formatter
	...
	try:
		...
		write_output("\t".join(
			['foi_name', 'foi_obs', 'n_fois', 'bg_obs', 'n_bgs', 'odds_ratio', "ci_lower", "ci_upper",
			 'shrunken_odds_ratio', str(stat_test) + '_ p_val', 'p_rand' if run_randomization_test else "",
		...
		for gf in gfs:
			...
			for i in range(len(good_fois)):
				[pvalue, shrunken_or] = output_p_value(res[i]["intersectregions"], res[i]["queryregions"], bg_obs,
													   ...
													   run_randomization_test, stat_test=stat_test, progress=curr_prog)
...
def main():
	...
	parser = argparse.ArgumentParser(
		description="Enrichment analysis of several sets of SNPs (FOIs) files against several genomic features (GFs). Example: python hypergeom4.py foi_full_names.txt gf_full_names.txt /path_to_background/snp137.bed.gz")
